...
import { ActionNotificationComponent,
  ...
  ConfirmDialog,
} from '@core-ui/views/partials/content/crud';
...
export class LayoutUtilsService {
	 ...
	 */
	deleteElement(title: string = '', description: string = '', waitDesciption: string = '', buttons?: { okText: string, cancelText: string }) {
		return this.dialog.open(DeleteEntityDialogComponent, {
			data: { title, description, waitDesciption, buttons },
			width: '440px'
		});
	}
  ...
  /**
   * Show Confirmation Archive (Mat-Dialog) before Entity Archive
   *
   * @param modelName: string
   */
  archiveElement(modelName: string) {
    const _title = 'WARNING';
    const _description = `Are you sure you want to archive ${modelName}?`;
    const _waitDesciption = `${modelName} is archiving...`;
    const _button = {
      okText: 'Archive', cancelText: 'Cancel'
    };

    return this.dialog.open(DeleteEntityDialogComponent, {
			data: { title: _title, description: _description, waitDesciption: _waitDesciption, buttons: _button },
			width: '440px'
		});
  }

  /**
	 * Showing Confirm action dialog.
	 *
	 * @param title: string
	 * @param message: string[]
	 */
	confirmDialog(title, message, confirmTxt?) {
		return this.dialog.open(ConfirmDialog, {
			data: { title, message, confirmTxt },
			width: '480px'
		});
  }
}
