...
export class FileManagerComponent implements OnInit {
  ...
  openFilemanagerButtonLabel: string = 'Open';

  selectedNode: NodeInterface;
  selectedNode$ = new BehaviorSubject<NodeInterface>(null);
  sideMenuClosed = true;
  ...
  ngOnInit() {
    ...
    this.store
      .pipe(select(state => state[STORE_NAME].fileManagerState.isLoading))
      .subscribe((data: boolean) => {
        this.loading$.next(data);
      });

    this.store
      .pipe(select(state => state[STORE_NAME].fileManagerState.selectedNode))
  ...
  handleUploadDialog(event: any, isCreateFolder = false) {
